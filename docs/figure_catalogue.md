...
| 4.1 | 218–222 | **T3** | Near-tritronquée: two pole fields + smooth imaginary-axis band; BVP solver path shown | u(0), u'(0), u(20i), u'(20i) recovered to ≤1e-10 abs; complete pole field's pole locations agree with FW Table 5.1 reference values. **PARTIAL — step (i) BVP pin shipped in `test/fw_fig_41_test.jl` (bead `padetaylor-0c3`): u(0) ≤3.5e-13, u'(0) ≤5.3e-11, u(±20i) = imposed BC (exact), Schwarz symmetry on imag axis ≤1e-12 — see worklog 016. Steps (ii) outward IVP run-out + (iii) interior smooth-band fill are downstream work; no follow-up bead filed (the lattice dispatcher v1 covers the line-190 horizontal-row variant of step (iii), and step (ii) is `path_network_solve` with the BVP-derived ICs).** |
