...
| **T5** | FFW2017 Fig 2, 3, 7 | T4 + SheetTracker + branch-cut routing | **PARTIAL — Fig 2 SHIPPED (worklog 044, bead `padetaylor-n93`). Infrastructure ALL SHIPPED: η-plane PVI eq. via A3 (worklog 041, bead `padetaylor-riu`, `pVI_eta_transformed_rhs` + `:transformed_eta` frame); constrained-wedge routing via A4 (worklog 042, ADR-0013, bead `padetaylor-bho`, new `src/BranchTracker.jl` with `branch_points` / `branch_cut_angles` / `cross_branch` kwargs on `path_network_solve` + `visited_sheet` per-branch counter); sheet-aware Stage-2 via A5 (worklog 043, bead `padetaylor-hed`, `grid_sheet` kwarg + `eval_at_sheet` accessor). Fig 3 (`padetaylor-a1l`) blocked by Fig 2 → now unblocked; Fig 7 (`padetaylor-mgx`) independent.** |
