...
| **T5** | FFW2017 Fig 2, 3, 7 | T4 + SheetTracker + branch-cut routing | **PARTIAL — Figs 2 + 3 SHIPPED (worklogs 044 + 046, beads `padetaylor-n93` + `padetaylor-a1l`). Infrastructure ALL SHIPPED: η-plane PVI eq. via A3 (worklog 041, bead `padetaylor-riu`, `pVI_eta_transformed_rhs` + `:transformed_eta` frame); constrained-wedge routing via A4 (worklog 042, ADR-0013, bead `padetaylor-bho`, new `src/BranchTracker.jl` with `branch_points` / `branch_cut_angles` / `cross_branch` kwargs on `path_network_solve` + `visited_sheet` per-branch counter); sheet-aware Stage-2 via A5 (worklog 043, bead `padetaylor-hed`, `grid_sheet` kwarg + `eval_at_sheet` accessor). Fig 3 ships zero new `src/` code — pure composition of A1+A2+A4+A5+A6 on a wider ζ-window with phase-portrait rendering. Fig 7 (`padetaylor-mgx`) independent.** |
