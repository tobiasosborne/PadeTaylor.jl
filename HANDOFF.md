...
| **Diagnostics** | `Diagnostics.quality_diagnose` quality certificate (ADR-0016) — loop-closure ΔP_rel per non-tree Delaunay edge on sheet 0; opt-in via `diagnose=true` kwarg; weak-dep on `DelaunayTriangulation` (extension) | ✅ **shipped** (worklog 048, bead `padetaylor-5t4`); motivated by trimodal distribution in `external/probes/loop-closure-fig1/REPORT.md:34-77` (6.3 % catastrophic edges on FFW Fig 1 sheet 0); `PathNetworkSolution` gains `diagnostics` field with backward-compat constructors; mutation-prove A done (18 s isolation); full `Pkg.test()` pending OOM-friction resolution (see worklog 048) | +32 DG.* |
| **Lattice dispatch v3** | `lattice_dispatch_solve` switched to `edge_gated_pole_field_solve` default + `strict::Bool` kwarg + `:bvp_fail` tag (ADR-0017, bead `padetaylor-0tj`) — fixes BC-corruption divergence on PI tritronquée at large N (121×121/[-20,20]², `‖Δu‖_∞=0.4458` vs `tol=1.8e-12`); smooth cells outside gated field are `NaN+NaN·im` (Rule 1); manual `mask` kwarg API preserved | ✅ **shipped** (worklog 049, bead `padetaylor-0tj`); edge-gated default is FW2011 md:401 made executable; mutation-X done (23.4 s isolation); LD.1.1 assertion adjusted (semantic flip: `bvp_solutions ≤ 5` under new default); full `Pkg.test()` pending OOM-friction (see worklog 049) | +61 LD.X.* (+ LD.1.1 adjustment) |
| **API audit** | Holistic public-API audit (`docs/api-review-2026-05-16.md`, bead `padetaylor-qur`) — 12 spawned follow-up beads (3 P2 v1.0-blocking: step-size kwarg rename `h_max`/`h_path` → `h` → `padetaylor-xds`; max-iter normalisation `maxiter` → `max_iter` → `padetaylor-0xn`; `pii_*`/`piv_entire` export omission → `padetaylor-gvz`; 5 P3 cosmetic; 4 P4 defer).  Output is advisory; no source changes. | ✅ **shipped** (worklog 050, bead `padetaylor-qur`, commit 9296731) | — (advisory) |
| **Heun module** | First-class Heun (`heun_g`, `heun_c`) — DLMF 31.2 + 31.12 (ADR-0018) | ✅ **shipped** (worklog 051) — Stages 0–5 complete; Stages 4c (Teukolsky QNM demo) + 6 (figures/README) deferred | +49 HE.* |
| **v0.2 literature scope** | Stage 0+ kickoff — vector / multi-component Painlevé-type systems (PRD lines 213–424). One Opus scout + 3 parallel Sonnet acquisition agents. Output: `docs/v0p2_literature_scope.md` (435 lines, 12 Tier-1 + 20 Tier-2 + 15 Tier-3 papers across 7 pillars), 7 new `references/<cluster>/` directories, 6 PDFs (Mano–Tsuda 2017, Cosgrove 2000-6 via `ps2pdf`, Beckermann–Labahn 1992/1994, Gonnet–Pachón–Trefethen 2011, Krichever 1980), 1 sample chapter (Trogdon–Olver 2016), 9 metadata stubs (4 books + Sakai 2001 + Novokshenov 2014 + 2 software + AMM 1977 + Kudryashov 1997). Surfaced 3 PRD citation corrections (lines 402, 403, 405) and 2 missed-by-PRD keystone papers (Mano–Tsuda 2017, Novokshenov 2014). | ✅ **shipped wave 1 (worklog 052)** | — (research artefacts only) |
| **v0.2 Batch 1 + prior-art sweep** | Second wave — serialised one-subagent-at-a-time per user instruction. (a) Batch 1 arXiv retry: 32/32 PDFs verified (Kapaev–Klein–Grava 2015 PI⁽²⁾ tritronquée, Noumi–Yamada 1998 founding A_n⁽¹⁾ paper, Nakatsukasa–Sète–Trefethen AAA, full PIII/PV/PVI / hierarchy / Garnier / RH-numerical corpus). (b) Prior-art second-wave sweep across 5 gaps (Cuyt school, van Iseghem, Aptekarev/Stahl, Klein/Bothner/Lisovyy/Joshi 2024–26, GitHub code search): **"first vector Padé–Taylor + Painlevé application" claim survives all five**. Two qualifications: **Amore 2021** (`arXiv:2111.12198`, Physica D) uses the exact name "Padé–Taylor method" for a scalar van der Pol integrator — v0.1 docs should acknowledge as parallel scalar prior art; **Adler–Sokolov 2025** (`arXiv:2512.18828`, J. Geom. Phys.) is a 2025 vector-Painlevé classification paper that reinforces v0.2 motivation rather than scooping it. +6 PDFs (4 Aptekarev convergence-theorem chain: López Lagomasino 2019 + Fidalgo-Lagomasino-Medina 2013 + Lagomasino-Medina 2014 + Ikonomov-Suetin 2026; Adler-Sokolov 2025; Amore 2021), +5 metadata stubs (Calgo 766, Cuyt 1982, Van Iseghem 1987, Aptekarev–Stahl 1992, Amore 2021 companion). | ✅ **shipped wave 2 (worklog 053)** | — (research artefacts only) |
...
Chebyshev-Newton BVP solver, ADR-0023); `src/Laplace2D.jl` (in-house
2D-Chebyshev spectral Laplace solver, ADR-0024); `VectorPathNetwork` Stage-2
fine-grid fill; and the headline figure
`figures/kkg_pi2_tritronquee_surface.jl` — the P_I^(2) tritronquée V_0(x,0)
over the complex plane as Re/Im heatmaps + 3D surfaces (KKG 2015 Figs
7.4/7.5), the pole-free sector computed THREE independent ways (ray-fan BVP /
2D-Chebyshev Laplace / Gridap FEM) and majority-voted per grid point. Full
suite **4712 / 4712 GREEN**. See worklogs
`056-vector-bvp-and-v8b-figure.md` (the VectorBVP solver + V8b — headline
lesson: the V8b "separatrix" blocker was a sign error in
`pI2_tritronquee_ic`, `bb1e208`) and `057-whole-plane-kkg-surface-figure.md`
(the surface figure — lesson: a heavy package weak-dep, Gridap, broke the
build; re-homed to the `figures/` project). Worklog 055 covers the V1–V8a
vector stack. **The only remaining planned epic phase is V9**
(`padetaylor-0ln.19`) — v0.2 docs + release prep: README/CHANGELOG/RESEARCH
v0.2 sections, ADR review (ADR-0023 + ADR-0024 are new), HANDOFF refresh,
the `padetaylor-0ln` epic close-out. Open deferred beads: `0ln.21`/`0ln.22`
(V7 sophistications), `0ln.23` (shared-Q wedge selector — would sharpen the
headline figure's pole wedge), `0ln.27` (ChebUtil `_chebyshev_D1`
de-duplication). The `figures/` project now carries Gridap 0.20.7; the
surface figure + its test run under `--project=figures`. **Older context
below.** The prior pickup note: v0.2 literature
...
**Next work item**: API audit complete (worklog 050, bead `padetaylor-qur`).  The v1.0 normalisation work queue is now explicit — three P2 beads from the audit sit at the top: (a) **`padetaylor-xds`** (canonical `h` rename — highest user-visibility; `solve_pade`'s `h_max` + `lattice_dispatch_solve`'s `h_path` → `h` with deprecation shims); (b) **`padetaylor-0xn`** (`max_iter` normalisation — rename `bvp_solve`'s `maxiter` → `max_iter`, cascade through `dispatch_solve`'s `bvp_maxiter` → `bvp_max_iter`); (c) **`padetaylor-gvz`** (one-line export fix: add `pii_rational`, `pii_airy`, `piv_entire` to `src/PadeTaylor.jl` export block).  After those three the API is v1.0-ready on the naming front.  Pre-audit P2s remain: `padetaylor-ykg` (lift the FFW Fig 5 `~10⁻²` v1 gap), `padetaylor-zwh` (widen FFW Fig 1), `padetaylor-qum` (split `PathNetwork.jl` for Rule 6 LOC cap — now at ~1010 LOC).
